...
import { ALL_MOVES, Move, Outcome } from './ai/types';
import { resolveOutcome } from './ai/rules';
import { createAdaptiveAI, chooseMove, updateModel } from './ai/ai';
import { RoundLogger, exportToCSV, exportToJSON } from './ai/logger';
...
export function runAllSimulations() {
  const targetDir = path.resolve(process.cwd(), 'docs/report-data');
